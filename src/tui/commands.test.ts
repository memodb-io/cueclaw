...
describe('command execution', () => {
  ...
  it('/list shows no workflows message when empty', () => {
    const { ctx, messages } = makeContext(db)
    findCommand('list')!.execute('', ctx)
    expect(messages).toHaveLength(1)
    expect(messages[0].text).toContain('No workflows')
  })

  it('/list shows workflows', () => {
    const wf = makeTestWorkflow()
    insertWorkflow(db, wf)
    const { ctx, messages } = makeContext(db)
    findCommand('list')!.execute('', ctx)
    expect(messages).toHaveLength(1)
    expect(messages[0].text).toContain('Test Workflow')
  })

  it('/status shows workflow details', () => {
    const wf = makeTestWorkflow()
    insertWorkflow(db, wf)
    const { ctx, messages } = makeContext(db)
    findCommand('status')!.execute('wf_test123', ctx)
    expect(messages).toHaveLength(1)
    expect(messages[0].text).toContain('Test Workflow')
    expect(messages[0].text).toContain('wf_test123')
  })

  it('/status with no args falls back to /list', () => {
    const { ctx, messages } = makeContext(db)
  ...
  it('/clear empties messages', () => {
    const { ctx, messages } = makeContext(db)
    messages.push({ role: 'user', text: 'hello' })
